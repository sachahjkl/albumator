...
volume_name="albumator-${namespace}-data"
...
job_status="$(nomad job status -namespace "$namespace" -json albumator 2>/dev/null | jq -r '.[0].Status' || true)"
if [[ -n "$job_status" && "$job_status" != dead ]]; then
  printf 'stop the %s albumator Nomad job before restoration\n' "$namespace" >&2
  exit 1
fi
...
install -o 65532 -g 65532 -m 0600 "$archive" "$volume_path/local.db"
